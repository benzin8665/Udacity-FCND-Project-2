...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        # Task-1: read lat0, lon0 from colliders into floating point values
        ...
        # Task-2: set home position to (lon0, lat0, 0)
        self.set_home_position(lon0, lat0, 0)

        # Task-3: retrieve current global position
        global_pos = [self._longitude, self._latitude, self._altitude]

        # Task-4: convert to current local position using global_to_local()
        ...
        # Task-5: convert start position to current position rather than map center
        grid_start = (int(local_pos[0]-north_offset), int(local_pos[1]- east_offset))

        # Set goal as some arbitrary position on the grid
        # Task-6: adapt to set goal as latitude / longitude position and convert
        ...
        # Task-7: add diagonal motions with a cost of sqrt(2) to your A* implementation
        # or move to a different search space such as a graph (not done here)
        print('Local Start and Goal: ', grid_start, grid_goal)
        path, _ = a_star(grid, heuristic, grid_start, grid_goal)

        # Task-8: prune path to minimize number of waypoints
        ...
        # Task-9: send waypoints to sim (this is just for visualization of waypoints)
        self.send_waypoints()
